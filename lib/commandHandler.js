...
async function handleMessages(sock, { messages, type }) {
        ...
        if (isReplyToBot) {
            ...
            if (isSettingsReply) {
                ...
                const menuShortcuts = { 
                    ...
                    "9": "anime", "10": "economy", "11": "sports", "12": "religion",
                    "13": "dp", "14": "group", "15": "admin", "16": "system",
                    "17": "textmaker", "18": "owner"
                };
    ...
    try {
        ...
        const chatType = isGroup ? `Group (${jid.split("@")[0]})` : `Private (${senderNum})`;
        console.log(`📥 [COMMAND] .${commandName} | User: ${senderNum} | Chat: ${chatType}`);

        const allowed = await runMiddleware(context, command);
        if (!allowed) {
            console.log(`⚠️ [COMMAND BLOCKED] .${commandName} blocked by middleware | User: ${senderNum}`);
            return;
        }

        const sentMsg = await command.execute(context);
        const duration = Date.now() - startTime;
        console.log(`✅ [COMMAND SUCCESS] .${commandName} | Duration: ${duration}ms | User: ${senderNum}`);
        ...
        const duration = Date.now() - startTime;
        const senderNum = sender ? sender.split("@")[0] : "Unknown";
        console.error(`❌ [COMMAND FAILED] .${commandName} (${duration}ms) | User: ${senderNum} | Error:`, err.message || err);
        try {
