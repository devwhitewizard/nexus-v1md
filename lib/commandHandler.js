...
async function handleMessages(sock, { messages, type }) {
    ...
    try {
        ...
        // Mask middle digits: 254797715445 → 2547****445
        const maskNum = (num) => {
            if (!num || num.length < 7) return num;
            const keep = Math.min(4, Math.floor(num.length / 3));
            const tail = Math.min(3, Math.floor(num.length / 4));
            return num.slice(0, keep) + "****" + num.slice(-tail);
        };
        const maskedSender = maskNum(senderNum);
        const chatType = isGroup ? `Group (${jid.split("@")[0]})` : `Private (${maskedSender})`;
        console.log(`📥 [COMMAND] .${commandName} | User: ${maskedSender} | Chat: ${chatType}`);
        ...
        console.log(`✅ [COMMAND SUCCESS] .${commandName} | Duration: ${duration}ms | User: ${maskedSender}`);
