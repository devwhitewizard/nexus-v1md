...
// Check if sender is the configured SUDO user
const isSudo = (sender) => {
    if (!sender) return false;
    // Check explicit SUDO env var
    if (process.env.SUDO) {
        const senderDigits = sender.replace(/\D/g, "");
        const sudoDigits = process.env.SUDO.replace(/\D/g, "");
        if (sudoDigits && senderDigits.includes(sudoDigits)) return true;
    }
    // Fall back to owner check WITHOUT calling isOwner (avoids circular recursion)
    return _isOwnerCheck(sender);
};

// Internal owner check — does NOT call isSudo to prevent infinite recursion
const _isOwnerCheck = (sender) => {
    if (!sender) return false;
    const senderDigits = sender.replace(/\D/g, "");
...
// Public isOwner — checks SUDO first, then owner numbers
...
async function runMiddleware(ctx, command) {
    ...
    if (isBotAdminCheck && ctx.isGroup) {
        const checkBotIsAdmin = async () => {
            try {
                const metadata = await ctx.sock.groupMetadata(ctx.jid).catch(() => null);
                ...
                ].filter(Boolean).map(id => id.split(":")[0].toLowerCase()));

                const botDigits = Array.from(botIds).map(id => id.replace(/\D/g, "")).filter(Boolean);

                const groupAdminParticipants = metadata.participants.filter(p => p.admin === "admin" || p.admin === "superadmin");

                for (const adminP of groupAdminParticipants) {
                    const adminIdClean = adminP.id.split(":")[0].toLowerCase();
                    const adminDigits = adminIdClean.replace(/\D/g, "");

                    if (botIds.has(adminIdClean)) return true;
                    if (adminDigits && botDigits.some(bDig => bDig && (adminDigits === bDig || adminDigits.includes(bDig)))) return true;
                }
            } catch (e) {
                console.error("⚠️ Error checking bot admin status:", e.message);
            }
            return false;
        };

        let isBotGrpAdmin = await checkBotIsAdmin();

        // If false, re-fetch fresh group metadata once in case bot was recently promoted
        if (!isBotGrpAdmin) {
            try {
                await ctx.sock.groupMetadata(ctx.jid);
            } catch (_) {}
            isBotGrpAdmin = await checkBotIsAdmin();
        }

        if (!isBotGrpAdmin) {
            console.log(`🚫 Middleware: Blocked ${command.name} because bot is not admin in ${ctx.jid}`);
