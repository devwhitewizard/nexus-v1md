...
const isOwner = (sender) => {
    ...
    const isMatched = ownerNumbers.some(num => {
        ...
        // Match if raw digits are identical (ignores @lid vs @s.whatsapp.net)
        return ownerDigits && senderDigits.includes(ownerDigits);
    });

    // 🛡️ 3. LID specific backup check (if the sender is an LID, it might have a unique numeric chain)
    if (sender.includes("@lid")) {
        const lidMatch = ownerNumbers.some(num => num.includes("@lid") && num.split("@")[0] === sender.split("@")[0]);
        if (lidMatch) return true;
    }

    console.log(`🛡️ [Sec] Owner Check: ${sender} -> Result: ${isMatched || false}`);
