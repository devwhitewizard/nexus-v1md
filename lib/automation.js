...
const handleMessageDeleteOrEdit = async (sock, item, isUpsert = false) => {
    try {
        ...
        const origTime = new Date((original.timestamp || Date.now() / 1000) * 1000).toLocaleTimeString("en-GB", { timeZone });
        ...
        alertText += `*📍 Location:* ${isStatus ? "Status Story Update" : (isGroup ? "Group Chat" : "Private Chat")}\n\n`;
        const oldText = extractText(original.content);
        ...
        // 6. Dispatch Notification Targets
        const targets = new Set();
        if (global.myJid && !global.myJid.endsWith("@g.us")) targets.add(global.myJid);
        const primaryOwner = toJid(process.env.SUDO || ownerNumbers[0]);
        if (primaryOwner && !primaryOwner.endsWith("@g.us")) targets.add(primaryOwner);
        ...
        if (original.mediaPath && fs.existsSync(original.mediaPath)) {
            ...
            } else if (realType === "videoMessage" || ext === ".mp4") {
