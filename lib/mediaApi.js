...
const mediaApi = {
    ...
    async ytDownload(url) {
        const apis = [
            // 🔗 Primary: HectorManuel Workers API
            async (u) => {
                const { data } = await axios.get(`https://yt-dl.officialhectormanuel.workers.dev/?url=${encodeURIComponent(u)}`, AXIOS_OPTS);
                if (data && (data.status || data.audio) && data.audio) {
                    return { url: data.audio, title: data.title };
                }
                throw new Error('HectorManuel API failed');
            },
